		...
		<div id="list-story" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="estDate" title="${message(code: 'story.estDate.label', default: 'When')}" />
						<g:sortableColumn property="placeOfStory.name" title="Where" />
					
						<g:sortableColumn property="description" title="${message(code: 'story.text.label', default: 'Description')}" />
				...
				<g:each in="${storyInstanceList}" status="i" var="storyInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${storyInstance.id}">${fieldValue(bean: storyInstance, field: "title")}</g:link></td>
					
						<td>${storyInstance.estDate}</td>
						
						<td>${storyInstance?.placeOfStory?.name}</td>
